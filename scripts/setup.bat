...
echo ║  Este script configura PostgreSQL y inicia los servidores     ║
echo ╚════════════════════════════════════════════════════════════════╝
echo.
...
psql -U postgres -d postgres -c "CREATE DATABASE pazzi_buns;" 2>nul
echo ✅ Base de datos lista (o ya existe)

REM Inicializar tablas
echo.
echo [3/5] Inicializando tablas...
psql -U postgres -d pazzi_buns -f "server\init.sql" >nul 2>&1
echo ✅ Tablas creadas
